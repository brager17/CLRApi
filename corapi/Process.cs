...
namespace Microsoft.Samples.Debugging.CorDebug
{
    ...
    public sealed class CorProcess : CorController, IDisposable, IMemoryReader
    {
        ...
        public DynamicCorModule[] Modules => AppDomains
            .Cast<CorAppDomain>()
            .SelectMany(x => x.Assemblies.Cast<CorAssembly>())
        ...
        public CorFunction GetFunction(int token)
        {
            var modules = AppDomains
                .Cast<CorAppDomain>()
                .SelectMany(x => x.Assemblies.Cast<CorAssembly>())
                .SelectMany(x => x.Modules.Cast<DynamicCorModule>())
                .Reverse()
            ...
            foreach (var module in modules)
            {
                try
                {
                    return module.GetFunctionFromToken(token);
                }
                catch
                {
                }
