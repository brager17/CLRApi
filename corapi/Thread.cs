...
namespace Microsoft.Samples.Debugging.CorDebug
{
    public struct CorActiveFunction
    {
        ...
        private DynamicCorModule m_module;

        internal CorActiveFunction(int ilOffset, CorFunction managedFunction, DynamicCorModule managedModule)
        {
            m_ilOffset = ilOffset;
        ...
        public CorActiveFunction[] GetActiveFunctions()
        {
            ...
            for (int i = 0; i < pcFunctions; ++i)
            {
                caf[i] = new CorActiveFunction((int)afunctions[i].ilOffset,
                                               ...
                                               afunctions[i].pModule == null ? null : new DynamicCorModule(afunctions[i].pModule)
                                               );
            }
            return caf;
        }
    ...
    public sealed class CorFunction : WrapperBase
    {
        ...
        public DynamicCorModule Module
        {
            get
            {
                ...
                return (imodule == null ? null : new DynamicCorModule(imodule));
            }
        }
